...
local function run(msg, matches)
  if matches[1]:lower() == 'quick' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /quick ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "quick" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^[!/]([Ss]tatslist)$",
    "^[!/]([Ss]tats) (group) (%d+)",
    "^[!/]([Ss]tats) (boomrange)",-- Put everything you like :)
		"^[!/]([Bb]oomrange)"-- Put everything you like :)
    }, 
  run = run
}
